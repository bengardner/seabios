...
static inline __attribute__((always_inline)) void write32(volatile void *addr, u32 value)
{
 ...
 */
static int wait_tx_fifo(u8 *base_adr)
{
	int i;
	...
	if (read32(base_adr + I2C_ABORT_SOURCE) & 0x1ffff) {
		...
		return I2C_ERR_ABORT | (read32(base_adr + I2C_ABORT_SOURCE) & 0x1ffff);
	}
 ...
 * FIFO stall due to transmit abort will be checked and resolved
 */
static int wait_rx_fifo(u8 *base_adr)
{
	int i;
	if (read32(base_adr + I2C_ABORT_SOURCE) & 0x1ffff) {
		/* Reading back I2C_CLR_TX_ABRT resets abort lock on TX FIFO */
 ...
 */
static int wait_for_idle(u8 *base_adr)
{
	int i;
	volatile int status;
...
static u8 *i2c_get_base(struct pci_device *pdev)
{
	if (pdev) {
		u8 *base_ptr = (u8 *)pci_config_readl(pdev->bdf, PCI_BASE_ADDRESS_0);
		if (base_ptr != NULL)
			return base_ptr;
		dprintf(1, "I2C: Invalid Base address\n");
	}
	return NULL;
}
 ...
 */
int dw_i2c_init(struct pci_device *pdev)
{
	u8 *base_ptr = i2c_get_base(pdev);

	if (!base_ptr)
		return I2C_ERR;

	/* TODO: verify class 0x0c80? */

	/* Get memory base */
	base_ptr = (u8 *)pci_config_readl(pdev->bdf, PCI_BASE_ADDRESS_0);
	if (base_ptr == NULL) {
		dprintf(1, "I2C: Invalid Base address\n");
	...
	/* Enable memory space */
	pci_config_writel(pdev->bdf, PCI_COMMAND,
			   (pci_config_readl(pdev->bdf, PCI_COMMAND) | PCI_COMMAND_MEMORY));
	...
	write32(base_ptr + I2C_ENABLE, 0x01);
 ...
 */
int dw_i2c_read(struct pci_device *pdev, u8 chip, u8 addr, u8 *buf, u8 len)
{
	int i;
	u32 val;
	int stat;
	...
	write32(base_ptr + I2C_TARGET_ADR, chip);
	/* Send address inside slave to read from */
	write32(base_ptr + I2C_DATA_CMD, addr);
 ...
 * @return     I2C_SUCCESS when read was successful, otherwise error code
 */
int dw_i2c_write(struct pci_device *pdev, u8 chip, u8 addr, const u8 *buf, u8 len)
{
	int i;
	u32 val;
	int stat;
